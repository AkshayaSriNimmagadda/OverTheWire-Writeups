...
The goal of this level is to find the password for the next level. The password is obtainied using the following properties
...
Since they mentioned that the file is located somewhere on the server, so I started with the root directory `/`.
...
* `-user bandit7` → Finds the files owned by the user `bandit7`.
* `-group bandit6` → Finds the files belonging to the group `bandit6`.
* `-size 33c` → Finds files that are exactly 33 bytes in size. 
* `2>/dev/null` → Hides error messages such as -Permission denied- while searching.

---

### Step 3: Read the File and Obtain the Password

The command returned a file that matched all the required properties. So, I used the `cat` command to display its contents.

```bash
cat /var/lib/dpkg/info/bandit7.password 
```

The command displayed the **PASSWORD** for Bandit Level 7.
...
* **`-group`** — Searching for files owned to a specific group.
